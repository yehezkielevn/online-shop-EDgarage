            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div>
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        {{-- Tombol Add to Cart/Login --}}
                        @if($product->status === 'tersedia')
                            @auth
                                @if($product->isInCart())
                                    <a href="{{ route('cart') }}" class="w-full bg-gray-700 hover:bg-gray-600 text-white font-bold py-4 rounded-xl shadow-lg transition transform hover:scale-[1.02] flex items-center justify-center gap-2">
                                ...
                                @else
                                    <form action="{{ route('cart.add', $product->id) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-4 rounded-xl shadow-lg shadow-red-900/20 transition transform hover:scale-[1.02] flex items-center justify-center gap-2">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @endif
                            @else
                                <a href="{{ route('login') }}" class="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-4 rounded-xl shadow-lg shadow-red-900/20 transition transform hover:scale-[1.02] flex items-center justify-center gap-2">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @endauth
                        @else
                            <div class="w-full bg-gray-800 text-gray-400 font-bold py-4 rounded-xl shadow-lg flex items-center justify-center gap-2 cursor-not-allowed">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"></path>
                                </svg>
                                @if($product->status === 'pending_checkout')
                                    Sedang Diproses
                                @else
                                    Sudah Terjual
                                @endif
